...
class ChatSessionWithTracing:
    ...
    async def chat_with_gemini(self, message: dict, history: List, past_messages: List) -> Tuple[str, List, str]:
        ...
        """
        # Create a tracing span for this chat turn, as a child of the conversation span
        with tracer.start_as_current_span(
                ...
                # MODERATION STEP 2: Check media files
                    ...
                    for file_path in value:
                        ...
                        try:
                            ...
                            # Wrap file bytes into a BinaryContent so it can be included in the prompt to the AI
            ...
            try:
                with tracer.start_as_current_span("llm_customer"):
                    ...
                    # NOTE: pass past_messages as message_history so the agent keeps context across turns.
        ...
        with gr.Row():
            ...
            with gr.Column(scale=3):
                gr.ChatInterface(
                    ...
                    # Pass past_messages_state as additional_inputs/outputs so Pydantic AI's
                    # message history is preserved across turns, and feedback_display is
                    # populated with the second value returned by chat_with_gemini.
